...
// Has been taken from: https://github.com/greycubesgav/sqlite-phhammdist
...
static void phhammdist(sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    if (argc != 2) {
        sqlite3_result_error(context,
                             "phhammdist: requires 2 TEXT or 2 INTEGER arguments.", -1);
        return;
    }
    ...
    if (datatype_arg1 == SQLITE3_TEXT && datatype_arg2 == SQLITE3_TEXT) {
        ...
        // Therefore we need to convert any SIGNED ints (-7726118085807353866) into
        // UNSIGNED ints (10720625987902197750) before checking the distance between the
        // two
        ...
        sqlite3_result_error(context,
                             "phhammdist: both arguments need to be of type TEXT or both "
                             "arguments need to be of type INTEGER.",
                             -1);
        return;
    }
#ifdef DEBUG
